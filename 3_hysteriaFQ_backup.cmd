...
if not exist "%~dp0hysteria" (
  echo [错误] 未找到目录：%~dp0hysteria
  echo 请先完整解压到本地磁盘，不要在压缩包内直接运行。
  pause
  exit /b
)
if not exist "%~dp0hysteria\hysteria-tun-windows-6.0-386.exe" (
  echo [错误] 未找到：%~dp0hysteria\hysteria-tun-windows-6.0-386.exe
...
title Hysteria One-Click Launcher
