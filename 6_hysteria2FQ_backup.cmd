...
if not exist "%~dp0hysteria2" (
  echo [错误] 未找到目录：%~dp0hysteria2
  echo 请先完整解压到本地磁盘，不要在压缩包内直接运行。
  pause
  exit /b
)
if not exist "%~dp0hysteria2\hysteria2.exe" (
  echo [错误] 未找到：%~dp0hysteria2\hysteria2.exe
...
title Hysteria2 One-Click Launcher
...
start "" /D "%~dp0hysteria2" "%~dp0hysteria2\hysteria2.exe" -c "%~dp0hysteria2\config.json"
