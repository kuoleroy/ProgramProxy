...
if not exist "%~dp0shadowquic" (
  echo [错误] 未找到目录：%~dp0shadowquic
  echo 请先完整解压到本地磁盘，不要在压缩包内直接运行。
  pause
  exit /b
)
if not exist "%~dp0shadowquic\shadowquic.exe" (
  echo [错误] 未找到：%~dp0shadowquic\shadowquic.exe
...
title ShadowQuic One-Click Launcher
...
:ip4
start /wait "" "%~dp0shadowquic\ip_Update\ip_4.bat"
goto startfq

:ip3
start /wait "" "%~dp0shadowquic\ip_Update\ip_3.bat"
goto startfq

:ip2
start /wait "" "%~dp0shadowquic\ip_Update\ip_2.bat"
goto startfq

:ip1
start /wait "" "%~dp0shadowquic\ip_Update\ip_1.bat"
goto startfq

:startfq

echo Waiting for proxy to start, please wait...
start "" /D "%~dp0shadowquic" "%~dp0shadowquic\shadowquic.exe" -c "%~dp0shadowquic\client.yaml"
IF EXIST %~dp0Browser\chrome.exe (
    start %~dp0Browser\chrome.exe --user-data-dir=%~dp0chrome-user-data --proxy-server="socks5://127.0.0.1:4080"  https://www.google.com/
) ELSE (
	%SystemRoot%\System32\reg.exe query "HKLM\Software\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe" >nul 2>&1
	IF  not errorlevel 1 (
		start chrome.exe --user-data-dir=%~dp0chrome-user-data --proxy-server="socks5://127.0.0.1:4080"  https://www.google.com/
	) else (
