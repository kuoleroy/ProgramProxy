...
if not exist "%~dp0naiveproxy" (
  echo [错误] 未找到目录：%~dp0naiveproxy
  echo 请先完整解压到本地磁盘，不要在压缩包内直接运行。
  pause
  exit /b
)
if not exist "%~dp0naiveproxy\naive.exe" (
  echo [错误] 未找到：%~dp0naiveproxy\naive.exe
...
title NaiveProxy One-Click Launcher
...
start "" /D "%~dp0naiveproxy" "%~dp0naiveproxy\naive.exe" "%~dp0naiveproxy\config.json"
